...
function validateTheme(theme: Partial<Theme>): ThemeValidationError[] {
  ...
  if (!theme.colors) {
    ...
    // Validate color format for existing colors only
...
function sanitizeTheme(theme: any): Partial<Theme> {
  ...
  if (theme.name !== undefined) {
    result.name = theme.name?.toString().toLowerCase().replace(/[^a-z0-9-_]/g, '') || '';
  }
  
  if (theme.displayName !== undefined) {
    result.displayName = theme.displayName?.toString().trim() || '';
  }
  
  if (theme.description !== undefined) {
    result.description = theme.description?.toString().trim() || '';
  }
  ...
  if (theme.isPublic !== undefined) {
    result.isPublic = Boolean(theme.isPublic);
  }
  
  if (theme.tags !== undefined) {
    result.tags = Array.isArray(theme.tags) ? theme.tags.filter((tag: any) => typeof tag === 'string' && tag.trim()) : [];
  }
  
  if (theme.authorId !== undefined) {
    result.authorId = theme.authorId?.toString() || null;
  }
  
  if (theme.authorName !== undefined) {
    result.authorName = theme.authorName?.toString() || null;
  }
...
export async function exportThemeById(id: string): Promise<ApiResponse<any>> {
  try {
    ...
    const exportData = {
      ...
      theme: {
        ...theme,
        // Remove server-specific data for export but keep ID for reference
        authorId: undefined,
